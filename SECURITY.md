...
Use the private reporting contact published by the deployed site, with the subject `IAM-K1taru security report`.
...
The current custom-domain deployment, the GitHub Pages deployment, and the latest `main` branch are supported. Linked project deployments are separate applications with their own security boundaries; a link from this portfolio does not place those applications inside this policy.
